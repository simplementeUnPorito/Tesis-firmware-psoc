...
#define CAL_ADC_CAPTURE_CHANNEL PSOC_AMUX_CH_LP
#endif
 ...
 *
 * Medido en la placa el 2026-09-05, en uV por codigo sobre el tap final LPo
 * (antes ch3, desde el mapa de seis entradas ch4), que es el tap que se
 * captura y el unico cuyo error importa para esta secuencia:
 *
 *     ADDER -> LPo   3823        <- GRUESO: recorre toda la excursion
 *     LP    -> LPo    525        <- FINO: los ultimos milivoltios
 *     BP    -> LPo   1002        }  no son actuadores del LP:
 *     PGA   -> LPo   1026 x gan  }  son PERTURBACIONES
 *
...
static const PsocCalStage g_psoc_cal_stages[] = {
    ...
    { "GEO_PGA",    PSOC_AMUX_CH_PGA, CAL_TARGET_COUNTS_GEO_PGA, CAL_DIRECTION_GEO_PGA, CAL_DAC_CENTER_GEO_PGA, CAL_DAC_MAX_CHANGE_GEO_PGA, CAL_DAC_MAX_CHANGE_NEG_GEO_PGA, cal_vdac_geo_pga, 0u, 0u },
#if defined(VDAC_ref_BP_DEFAULT_DATA) || defined(CY_DVDAC_VDAC_ref_BP_H)
    { "GEO_BP",     PSOC_AMUX_CH_BP, CAL_TARGET_COUNTS_GEO_BP,  CAL_DIRECTION_GEO_BP,  CAL_DAC_CENTER_GEO_BP,  CAL_DAC_MAX_CHANGE_GEO_BP, CAL_DAC_MAX_CHANGE_NEG_GEO_BP,  cal_vdac_geo_bp,  0u, 0u },
#endif
    ...
    /* TODO(cal-bp-guard-opa-sum): en una estrategia futura, optimizar BPo
     * (PSOC_AMUX_CH_BP) y usar OPA_SUMo (PSOC_AMUX_CH_OPA_SUM) como restriccion
     * anti-saturacion antes de aceptar el paso hacia SUMo. Por ahora se
     * conserva exactamente la secuencia validada y solo se desplaza LPo. */
    { "GEO_SUM_LP", PSOC_AMUX_CH_LP, CAL_TARGET_COUNTS_GEO_LP,  CAL_DIRECTION_GEO_SUM, CAL_DAC_CENTER_GEO_SUM, CAL_DAC_MAX_CHANGE_GEO_SUM, CAL_DAC_MAX_CHANGE_NEG_GEO_SUM, cal_vdac_geo_sum, 1u, 0u },
    { "GEO_LP",     PSOC_AMUX_CH_LP, CAL_TARGET_COUNTS_GEO_LP,  CAL_DIRECTION_GEO_LP,  CAL_DAC_CENTER_GEO_LP,  CAL_DAC_MAX_CHANGE_GEO_LP, CAL_DAC_MAX_CHANGE_NEG_GEO_LP,  cal_vdac_geo_lp,  1u, 1u },
};
