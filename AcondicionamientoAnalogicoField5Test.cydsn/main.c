...
#include "psoc_nv.h"
/* Autotest de placa. Solo existe en AcondicionamientoAnalogicoTest: todo
 * su contenido es static, asi que no hay que registrar un .c en el
 * .cyprj (que no se edita a mano). */
...
/* PROYECTO DE AUTOTEST: 16 en vez de 512. g_capture_raw es
 * PSOC_CAPTURE_MAX_BATCHES * 90 bytes = 46080 B con 512, sobre una SRAM de
 * 64 KB que en el firmware de campo queda al 79 %. El autotest solo necesita
 * lotes cortos (rampa de debug, unas pocas capturas de verificacion), asi que
 * bajarlo libera SRAM. No se baja mas porque el test del golpe al geofono
 * necesita ver el timbrado del SM-24 (fn = 10 Hz, zeta = 0,25): 128 lotes son
 * 3840 muestras = 1,47 s a 2604 Hz, suficiente para el pico y varios ciclos
 * de decaimiento. Con 16 lotes (184 ms) no se veria el decaimiento. */
#define PSOC_CAPTURE_MAX_BATCHES 128u
#endif
...
#endif
    st_sync_note_edge();   /* autotest B3: cuenta flancos si esta armado */
    if (SYNC_IN_Read())
    {
...
CY_ISR(unexpected_irq_trap)
{
    ...
    g_st_irq_traps++;   /* autotest C2 */
}

CY_ISR(hardfault_trap)
{
    g_st_hardfaults++;   /* autotest C2 */
        ...
        switch (rx_state)
        {
            ...
            case 1u:
                ...
                switch (rx)
                {
                    ...
                    case PSOC_CMD_PONG:
                    /* Autotest: si un opcode no figura aca, el frame se
                     * descarta EN SILENCIO. */
                    case PSOC_CMD_ST_REPORT:
                    case PSOC_CMD_ST_SYNC:
                    case PSOC_CMD_ST_MEAS_DC:
                    case PSOC_CMD_ST_MEAS_AC:
                        rx_cmd = rx; rx_state = 2u; break;
                    case 0xA3u: case PSOC_CMD_SD_READ_BATCH:
                    case PSOC_CMD_ST_SET_IDAC:   /* 2 params: etapa + codigo */
            ...
            case 3u:
                ...
                if (g_state == PSOC_CALIBRATING) {
                    switch (rx_cmd)
                    {
                        ...
                        case PSOC_CMD_SD_READ_BATCH:
                            ...
                            break;
                        /* Autotest: contestar con una trama 0xC5 de rechazo en
                         * vez de un CAL_BUSY generico, para que el ESP sepa
                         * exactamente que medicion no se hizo y la reintente. */
                        case PSOC_CMD_ST_REPORT:
                        case PSOC_CMD_ST_SYNC:
                        case PSOC_CMD_ST_SET_IDAC:
                        case PSOC_CMD_ST_MEAS_DC:
                        case PSOC_CMD_ST_MEAS_AC:
                            st_send_result(ST_ID_IDENTITY, ST_REJECTED,
                                           (int32)rx_cmd, (int32)g_state);
                            break;
                        default:
                            uart_send_diag(PSOC_EVT_CAL_BUSY, rx_cmd);
                            break;
                ...
                /* Las primitivas de autotest paran el ADC, conmutan el AMux
                 * y mueven referencias. Hoy no pueden llegar en plena captura
                 * porque service_runtime() ni siquiera llama a uart_service()
                 * en ARMED ni en SAMPLING, pero si un refactor futuro abriera
                 * esa ventana corromperian la captura en silencio. Sale mas
                 * barato negarse aca que descubrirlo en campo. */
                if ((rx_cmd == PSOC_CMD_ST_REPORT   ||
                     rx_cmd == PSOC_CMD_ST_SYNC     ||
                     rx_cmd == PSOC_CMD_ST_SET_IDAC ||
                     rx_cmd == PSOC_CMD_ST_MEAS_DC  ||
                     rx_cmd == PSOC_CMD_ST_MEAS_AC) &&
                    (g_state != PSOC_IDLE)) {
                    st_send_result(ST_ID_IDENTITY, ST_REJECTED,
                                   (int32)rx_cmd, (int32)g_state);
                    break;
                ...
                switch (rx_cmd)
                {
                    /* ---- Autotest de placa (proyecto ...Test) --------------
                     * El PSoC solo ejecuta primitivas atomicas y contesta una
                     * trama 0xC5 por medicion; el ESP32 secuencia y decide. */
                    case PSOC_CMD_ST_REPORT:
                        if (rx_p1 == 5u) {
                        ...
                        break;
                    case PSOC_CMD_ST_SYNC:
                        st_handle_sync(rx_p1);
                        break;
                    case PSOC_CMD_ST_SET_IDAC:
                        st_handle_set_idac(rx_p1, rx_p2);
                        break;
                    case PSOC_CMD_ST_MEAS_DC:
                        st_handle_meas_dc(rx_p1);
                        break;
                    case PSOC_CMD_ST_MEAS_AC:
                        st_handle_meas_ac(rx_p1);
                        break;
                    /* -------------------------------------------------------- */
...
int main(void)
{
    ...
    /* Recuperar también la ganancia asociada antes de aplicar sus cuatro DAC. */
