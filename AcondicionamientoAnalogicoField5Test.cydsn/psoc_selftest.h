 ...
 * ------------------------------
 * Todo el estimulo sale de los cuatro IDAC8 de referencia y toda la medicion
 * entra por los seis canales del AMux_ADC. El sexto canal se incorporo al
 * TopDesign el 2026-09-07 para observar el sumador antes de PGAout.
 *
 *   IDAC (etapa)         AMux_ADC (tap)
 *   0 = Vref_PGA          ch0 = salida de PGAgain
 *   1 = Vref_BP           ch1 = salida del pasabanda (BPo)
 *   2 = Vref_ADDER        ch2 = salida del sumador antes de PGAout (OPA_SUMo)
 *                         ch3 = salida despues de PGAout (SUMo)
 *   3 = Vref_LP           ch4 = salida del pasabajos (LPo)
 *                         ch5 = AMuxCapacitor (100 nF a Vss)
 *
...
static void st_handle_set_idac(uint8 stage_and_sign, uint8 magnitude)
{
    ...
    st_send_result(id, ST_OK, (int32)code,
                   psoc_idac_stage_code_to_uv_signed(stage, code));
}
