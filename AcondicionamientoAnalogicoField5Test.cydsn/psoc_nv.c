...
uint8 psoc_nv_save_for_gain(uint8 pga_code, const int16 *cal_dac, uint8 cal_count)
{
    ...
    row0[NV_PGAOUT_OFFSET] = psoc_hw_get_pgaout_code();
    row0[5] = valid_mask;
...
uint8 psoc_nv_load_pgaout_for_gain(uint8 pga_code, uint8 *pgaout_code,
                                  uint8 cal_count)
{
    int16 scratch[PSOC_NV_CAL_STAGES];
    uint16 base;
    uint8 stored;

    if (pgaout_code == (uint8 *)0 ||
        !psoc_nv_load_for_gain(pga_code, scratch, cal_count)) {
        return 0u;
    }
    base = nv_row_base(NV_ROW_FOR_GAIN(pga_code));
    stored = EEPROM_ReadByte((uint16)(base + NV_PGAOUT_OFFSET));
    if (stored > 8u) { return 0u; }
    *pgaout_code = stored;
    return 1u;
}
