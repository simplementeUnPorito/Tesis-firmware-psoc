...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_PC) || \
         ...
         
{
        ...
        */
        if(0u != PC_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = PC_GetRxBufferSize();
                CyDelay(PC_BYTE2BYTE_TIME_OUT);
            }
            while(PC_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  PC_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(PC_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(PC_GetRxBufferSize() > 0u)
    {
        tempCount = PC_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = PC_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional PC_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != PC_PACKET_EOP)
            {
                CyDelay(PC_WAIT_EOP_DELAY);
            }
        }
