...
/*******************************************************************************
* Function Name: Clock_Debouncer_Start
********************************************************************************
*
* Summary:
...
void Clock_Debouncer_Start(void) 
{
    ...
    Clock_Debouncer_CLKEN |= Clock_Debouncer_CLKEN_MASK;
	Clock_Debouncer_CLKSTBY |= Clock_Debouncer_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Debouncer_Stop
********************************************************************************
*
* Summary:
...
void Clock_Debouncer_Stop(void) 
{
    ...
    Clock_Debouncer_CLKEN &= (uint8)(~Clock_Debouncer_CLKEN_MASK);
	Clock_Debouncer_CLKSTBY &= (uint8)(~Clock_Debouncer_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Debouncer_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Debouncer_CLKEN & Clock_Debouncer_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Debouncer_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Debouncer_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Debouncer__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Debouncer_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Debouncer_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Debouncer_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Debouncer_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Debouncer_GetSourceRegister();
    uint16 oldDivider = Clock_Debouncer_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Debouncer_CLKEN & Clock_Debouncer_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Debouncer_DIV_PTR, clkDivider);
                Clock_Debouncer_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Debouncer_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Debouncer_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Debouncer_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Debouncer_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Debouncer_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Debouncer_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Debouncer_MOD_SRC |= modeBitMask & (uint8)Clock_Debouncer_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Debouncer_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Debouncer_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Debouncer_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Debouncer_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Debouncer_MOD_SRC & (uint8)(Clock_Debouncer_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Debouncer_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Debouncer_GetDividerRegister();
    uint8 oldSrc = Clock_Debouncer_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Debouncer_MOD_SRC |= CYCLK_SSS;
        Clock_Debouncer_MOD_SRC =
            (Clock_Debouncer_MOD_SRC & (uint8)(~Clock_Debouncer_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Debouncer_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Debouncer_MOD_SRC & Clock_Debouncer_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Debouncer_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Debouncer_PHASE = clkPhase & Clock_Debouncer_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Debouncer_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Debouncer_PHASE & Clock_Debouncer_PHASE_MASK;
}
