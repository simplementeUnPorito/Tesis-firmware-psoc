/*******************************************************************************
* File Name: VDAC_ref_BP_IntClock.c
* Version 2.20
*
*  Description:
...
#include "VDAC_ref_BP_IntClock.h"
...
/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_Start
********************************************************************************
*
* Summary:
...
void VDAC_ref_BP_IntClock_Start(void) 
{
    ...
    VDAC_ref_BP_IntClock_CLKEN |= VDAC_ref_BP_IntClock_CLKEN_MASK;
	VDAC_ref_BP_IntClock_CLKSTBY |= VDAC_ref_BP_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_Stop
********************************************************************************
*
* Summary:
...
void VDAC_ref_BP_IntClock_Stop(void) 
{
    ...
    VDAC_ref_BP_IntClock_CLKEN &= (uint8)(~VDAC_ref_BP_IntClock_CLKEN_MASK);
	VDAC_ref_BP_IntClock_CLKSTBY &= (uint8)(~VDAC_ref_BP_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((VDAC_ref_BP_IntClock_CLKEN & VDAC_ref_BP_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = VDAC_ref_BP_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = VDAC_ref_BP_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* VDAC_ref_BP_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(VDAC_ref_BP_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(VDAC_ref_BP_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = VDAC_ref_BP_IntClock_GetSourceRegister();
    uint16 oldDivider = VDAC_ref_BP_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = VDAC_ref_BP_IntClock_CLKEN & VDAC_ref_BP_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(VDAC_ref_BP_IntClock_DIV_PTR, clkDivider);
                VDAC_ref_BP_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                VDAC_ref_BP_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(VDAC_ref_BP_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((VDAC_ref_BP_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(VDAC_ref_BP_IntClock_DIV_PTR, clkDivider);
				VDAC_ref_BP_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(VDAC_ref_BP_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    VDAC_ref_BP_IntClock_MOD_SRC |= modeBitMask & (uint8)VDAC_ref_BP_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    VDAC_ref_BP_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(VDAC_ref_BP_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return VDAC_ref_BP_IntClock_MOD_SRC & (uint8)(VDAC_ref_BP_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = VDAC_ref_BP_IntClock_GetDividerRegister();
    uint8 oldSrc = VDAC_ref_BP_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        VDAC_ref_BP_IntClock_MOD_SRC |= CYCLK_SSS;
        VDAC_ref_BP_IntClock_MOD_SRC =
            (VDAC_ref_BP_IntClock_MOD_SRC & (uint8)(~VDAC_ref_BP_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return VDAC_ref_BP_IntClock_MOD_SRC & VDAC_ref_BP_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    VDAC_ref_BP_IntClock_PHASE = clkPhase & VDAC_ref_BP_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: VDAC_ref_BP_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return VDAC_ref_BP_IntClock_PHASE & VDAC_ref_BP_IntClock_PHASE_MASK;
}
