...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_PC) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_PC_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
        ...
        */
        if(0u != UART_PC_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_PC_GetRxBufferSize();
                CyDelay(UART_PC_BYTE2BYTE_TIME_OUT);
            }
            while(UART_PC_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_PC_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_PC_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    {
        tempCount = UART_PC_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_PC_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_PC_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_PC_PACKET_EOP)
            {
                CyDelay(UART_PC_WAIT_EOP_DELAY);
            }
        }
