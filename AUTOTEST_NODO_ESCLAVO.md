...
| Código | Qué prueba | Criterio implementado | Qué falla física implica un `FAIL` |
|---|---|---|---|
| D1 | Reposo DC de todos los taps analógicos. | `FAIL` si alguna media cae fuera de la ventana observable, 880,4 a 1122,7 mV de banco con 25 mV de margen. **Corregido el 2026-09-06**: el criterio era `|V| >= 2,3 V`, que en las unidades que mide el autotest NO SE ALCANZA NUNCA —los taps se sientan entre 0,96 y 1,12 V—, así que D1 daba `PASS` con la cadena entera saturada. Una medición individual que no llega no fuerza `FAIL` en este ítem. | Algún tap está contra un riel; el ítem no localiza por sí solo el componente. |
