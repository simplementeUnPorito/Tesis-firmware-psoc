...
uint8 Comp_SUM_initVar = 0u;

/* Internal functions definitoin */
static void Comp_SUM_trimAdjust(uint8 nibble) ;

/* static Comp_SUM_backupStruct  Comp_SUM_backup; */
#if (CY_PSOC5A)
    static Comp_SUM_LOWPOWER_BACKUP_STRUCT  Comp_SUM_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_SUM_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_SUM_Init
********************************************************************************
*
* Summary:
...
void Comp_SUM_Init(void) 
{
    ...
    Comp_SUM_SetSpeed(Comp_SUM_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_SUM_DEFAULT_HYSTERESIS == 0u )
        Comp_SUM_CR |= Comp_SUM_HYST_OFF;
    #else
        Comp_SUM_CR &= (uint8)(~Comp_SUM_HYST_OFF);
    #endif /* Comp_SUM_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_SUM_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_SUM_CR &= (uint8)(~Comp_SUM_PWRDWN_OVRD);
        #else 
            Comp_SUM_CR |= Comp_SUM_PWRDWN_OVRD;
        #endif /* Comp_SUM_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_SUM_CR |= Comp_SUM_MX_AO;

    /* Set default sync */
    Comp_SUM_CLK &= (uint8)(~Comp_SUM_SYNCCLK_MASK);
    #if ( Comp_SUM_DEFAULT_BYPASS_SYNC == 0u )
        Comp_SUM_CLK |= Comp_SUM_SYNC_CLK_EN;
    #else
        Comp_SUM_CLK |= Comp_SUM_BYPASS_SYNC;
    #endif /* Comp_SUM_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_SUM_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_SUM_PWRMGR |= Comp_SUM_ACT_PWR_EN;
    Comp_SUM_STBY_PWRMGR |= Comp_SUM_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_SUM_restoreReg == 1u)
        {
            Comp_SUM_CR = Comp_SUM_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_SUM_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  Comp_SUM_initVar: Is modified when this function is called for the 
...
void Comp_SUM_Stop(void) 
{
    ...
    Comp_SUM_PWRMGR &= (uint8)(~Comp_SUM_ACT_PWR_EN);
    Comp_SUM_STBY_PWRMGR &= (uint8)(~Comp_SUM_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_SUM_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_SUM_lowPowerBackup.compCRReg = Comp_SUM_CR;
        Comp_SUM_CR = Comp_SUM_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_SUM_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_SUM_SetSpeed(uint8 speed) 
{
    ...
    Comp_SUM_CR = (Comp_SUM_CR & (uint8)(~Comp_SUM_PWR_MODE_MASK)) |
                           (speed & Comp_SUM_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_SUM_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_SUM_TR = Comp_SUM_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_SUM_TR0 = Comp_SUM_HS_TRIM_TR0;
            Comp_SUM_TR1 = Comp_SUM_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_SUM_TR = Comp_SUM_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_SUM_TR0 = Comp_SUM_LS_TRIM_TR0;
            Comp_SUM_TR1 = Comp_SUM_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_SUM_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_SUM_WRK & Comp_SUM_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_SUM_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_SUM_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_SUM_WRK & Comp_SUM_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_SUM_TR |= Comp_SUM_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_SUM_TR0 |= Comp_SUM_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_SUM_TR |= Comp_SUM_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_SUM_TR1 |= Comp_SUM_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_SUM_WRK & Comp_SUM_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Comp_SUM_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Comp_SUM_ZeroCal(void) 
{
    ...
    Comp_SUM_SW0 = 0x00u;
    Comp_SUM_SW2 = 0x00u;
    Comp_SUM_SW3 = tmpSW3 & (uint8)(~Comp_SUM_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_SUM_CR;
    Comp_SUM_CR |= (Comp_SUM_CAL_ON | Comp_SUM_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_SUM_TR = Comp_SUM_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_SUM_TR0 = Comp_SUM_DEFAULT_CMP_TRIM;
        Comp_SUM_TR1 = Comp_SUM_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_SUM_CR & Comp_SUM_PWR_MODE_MASK) == Comp_SUM_PWR_MODE_FAST)
    {
        Comp_SUM_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_SUM_trimAdjust(1u);
		Comp_SUM_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_SUM_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_SUM_TR1 << 8u) | (Comp_SUM_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_SUM_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_SUM_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_SUM_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_SUM_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
