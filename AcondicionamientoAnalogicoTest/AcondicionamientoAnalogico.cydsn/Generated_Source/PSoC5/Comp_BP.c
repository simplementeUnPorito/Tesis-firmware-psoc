...
uint8 Comp_BP_initVar = 0u;

/* Internal functions definitoin */
static void Comp_BP_trimAdjust(uint8 nibble) ;

/* static Comp_BP_backupStruct  Comp_BP_backup; */
#if (CY_PSOC5A)
    static Comp_BP_LOWPOWER_BACKUP_STRUCT  Comp_BP_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_BP_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_BP_Init
********************************************************************************
*
* Summary:
...
void Comp_BP_Init(void) 
{
    ...
    Comp_BP_SetSpeed(Comp_BP_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_BP_DEFAULT_HYSTERESIS == 0u )
        Comp_BP_CR |= Comp_BP_HYST_OFF;
    #else
        Comp_BP_CR &= (uint8)(~Comp_BP_HYST_OFF);
    #endif /* Comp_BP_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_BP_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_BP_CR &= (uint8)(~Comp_BP_PWRDWN_OVRD);
        #else 
            Comp_BP_CR |= Comp_BP_PWRDWN_OVRD;
        #endif /* Comp_BP_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_BP_CR |= Comp_BP_MX_AO;

    /* Set default sync */
    Comp_BP_CLK &= (uint8)(~Comp_BP_SYNCCLK_MASK);
    #if ( Comp_BP_DEFAULT_BYPASS_SYNC == 0u )
        Comp_BP_CLK |= Comp_BP_SYNC_CLK_EN;
    #else
        Comp_BP_CLK |= Comp_BP_BYPASS_SYNC;
    #endif /* Comp_BP_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_BP_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_BP_PWRMGR |= Comp_BP_ACT_PWR_EN;
    Comp_BP_STBY_PWRMGR |= Comp_BP_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_BP_restoreReg == 1u)
        {
            Comp_BP_CR = Comp_BP_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_BP_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  Comp_BP_initVar: Is modified when this function is called for the 
...
void Comp_BP_Stop(void) 
{
    ...
    Comp_BP_PWRMGR &= (uint8)(~Comp_BP_ACT_PWR_EN);
    Comp_BP_STBY_PWRMGR &= (uint8)(~Comp_BP_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_BP_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_BP_lowPowerBackup.compCRReg = Comp_BP_CR;
        Comp_BP_CR = Comp_BP_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_BP_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_BP_SetSpeed(uint8 speed) 
{
    ...
    Comp_BP_CR = (Comp_BP_CR & (uint8)(~Comp_BP_PWR_MODE_MASK)) |
                           (speed & Comp_BP_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_BP_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_BP_TR = Comp_BP_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_BP_TR0 = Comp_BP_HS_TRIM_TR0;
            Comp_BP_TR1 = Comp_BP_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_BP_TR = Comp_BP_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_BP_TR0 = Comp_BP_LS_TRIM_TR0;
            Comp_BP_TR1 = Comp_BP_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_BP_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_BP_WRK & Comp_BP_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_BP_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_BP_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_BP_WRK & Comp_BP_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_BP_TR |= Comp_BP_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_BP_TR0 |= Comp_BP_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_BP_TR |= Comp_BP_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_BP_TR1 |= Comp_BP_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_BP_WRK & Comp_BP_CMP_OUT_MASK) != cmpState)
        {
...
uint16 Comp_BP_ZeroCal(void) 
{
    ...
    Comp_BP_SW0 = 0x00u;
    Comp_BP_SW2 = 0x00u;
    Comp_BP_SW3 = tmpSW3 & (uint8)(~Comp_BP_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_BP_CR;
    Comp_BP_CR |= (Comp_BP_CAL_ON | Comp_BP_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_BP_TR = Comp_BP_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_BP_TR0 = Comp_BP_DEFAULT_CMP_TRIM;
        Comp_BP_TR1 = Comp_BP_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_BP_CR & Comp_BP_PWR_MODE_MASK) == Comp_BP_PWR_MODE_FAST)
    {
        Comp_BP_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_BP_trimAdjust(1u);
		Comp_BP_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_BP_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_BP_TR1 << 8u) | (Comp_BP_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_BP_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_BP_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_BP_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_BP_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
