...
uint8 Comp_LP_initVar = 0u;

/* Internal functions definitoin */
static void Comp_LP_trimAdjust(uint8 nibble) ;

/* static Comp_LP_backupStruct  Comp_LP_backup; */
#if (CY_PSOC5A)
    static Comp_LP_LOWPOWER_BACKUP_STRUCT  Comp_LP_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_LP_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_LP_Init
********************************************************************************
*
* Summary:
...
void Comp_LP_Init(void) 
{
    ...
    Comp_LP_SetSpeed(Comp_LP_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_LP_DEFAULT_HYSTERESIS == 0u )
        Comp_LP_CR |= Comp_LP_HYST_OFF;
    #else
        Comp_LP_CR &= (uint8)(~Comp_LP_HYST_OFF);
    #endif /* Comp_LP_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_LP_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_LP_CR &= (uint8)(~Comp_LP_PWRDWN_OVRD);
        #else 
            Comp_LP_CR |= Comp_LP_PWRDWN_OVRD;
        #endif /* Comp_LP_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_LP_CR |= Comp_LP_MX_AO;

    /* Set default sync */
    Comp_LP_CLK &= (uint8)(~Comp_LP_SYNCCLK_MASK);
    #if ( Comp_LP_DEFAULT_BYPASS_SYNC == 0u )
        Comp_LP_CLK |= Comp_LP_SYNC_CLK_EN;
    #else
        Comp_LP_CLK |= Comp_LP_BYPASS_SYNC;
    #endif /* Comp_LP_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_LP_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_LP_PWRMGR |= Comp_LP_ACT_PWR_EN;
    Comp_LP_STBY_PWRMGR |= Comp_LP_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_LP_restoreReg == 1u)
        {
            Comp_LP_CR = Comp_LP_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_LP_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  Comp_LP_initVar: Is modified when this function is called for the 
...
void Comp_LP_Stop(void) 
{
    ...
    Comp_LP_PWRMGR &= (uint8)(~Comp_LP_ACT_PWR_EN);
    Comp_LP_STBY_PWRMGR &= (uint8)(~Comp_LP_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_LP_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_LP_lowPowerBackup.compCRReg = Comp_LP_CR;
        Comp_LP_CR = Comp_LP_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_LP_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_LP_SetSpeed(uint8 speed) 
{
    ...
    Comp_LP_CR = (Comp_LP_CR & (uint8)(~Comp_LP_PWR_MODE_MASK)) |
                           (speed & Comp_LP_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_LP_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_LP_TR = Comp_LP_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_LP_TR0 = Comp_LP_HS_TRIM_TR0;
            Comp_LP_TR1 = Comp_LP_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_LP_TR = Comp_LP_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_LP_TR0 = Comp_LP_LS_TRIM_TR0;
            Comp_LP_TR1 = Comp_LP_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_LP_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_LP_WRK & Comp_LP_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_LP_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_LP_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_LP_WRK & Comp_LP_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_LP_TR |= Comp_LP_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_LP_TR0 |= Comp_LP_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_LP_TR |= Comp_LP_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_LP_TR1 |= Comp_LP_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_LP_WRK & Comp_LP_CMP_OUT_MASK) != cmpState)
        {
...
uint16 Comp_LP_ZeroCal(void) 
{
    ...
    Comp_LP_SW0 = 0x00u;
    Comp_LP_SW2 = 0x00u;
    Comp_LP_SW3 = tmpSW3 & (uint8)(~Comp_LP_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_LP_CR;
    Comp_LP_CR |= (Comp_LP_CAL_ON | Comp_LP_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_LP_TR = Comp_LP_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_LP_TR0 = Comp_LP_DEFAULT_CMP_TRIM;
        Comp_LP_TR1 = Comp_LP_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_LP_CR & Comp_LP_PWR_MODE_MASK) == Comp_LP_PWR_MODE_FAST)
    {
        Comp_LP_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_LP_trimAdjust(1u);
		Comp_LP_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_LP_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_LP_TR1 << 8u) | (Comp_LP_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_LP_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_LP_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_LP_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_LP_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
