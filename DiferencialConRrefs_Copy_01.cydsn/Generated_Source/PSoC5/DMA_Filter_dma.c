...
uint8 DMA_Filter_DmaInitialize(uint8 BurstCount, uint8 ReqestPerBurst, uint16 UpperSrcAddress, uint16 UpperDestAddress) 
{
    ...
    DMA_Filter_DmaHandle = (uint8)DMA_Filter__DRQ_NUMBER;

    /* Configure the channel. */
    (void)CyDmaChSetConfiguration(DMA_Filter_DmaHandle,
                                  BurstCount,
                                  ReqestPerBurst,
                                  (uint8)DMA_Filter__TERMOUT0_SEL,
                                  (uint8)DMA_Filter__TERMOUT1_SEL,
                                  (uint8)DMA_Filter__TERMIN_SEL);

    /* Set the extended address for the transfers */
    (void)CyDmaChSetExtendedAddress(DMA_Filter_DmaHandle, UpperSrcAddress, UpperDestAddress);

    /* Set the priority for this channel */
    (void)CyDmaChPriority(DMA_Filter_DmaHandle, (uint8)DMA_Filter__PRIORITY);
...
{
    /* Disable the channel */
    (void)CyDmaChDisable(DMA_Filter_DmaHandle);
}
